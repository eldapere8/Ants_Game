    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect) is True:
                    self.ant.contain_ant(insect)
                elif insect.can_contain(self.ant) is True:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while not (count <= self.max_range and count >= self.min_range):
            current_place = current_place.entrance
            count+=1    
        
        if current_place.name == "Hive":
            return None   
        while isinstance(current_place, Place) and not current_place.bees:
            current_place = current_place.entrance
            count+=1
        else:
            if isinstance(current_place, Place) and current_place.name == "Hive":
                return None
            elif count <= self.max_range and count >= self.min_range and isinstance(current_place, Place):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <=0:
            ...
            self.place.remove_insect(self)
            self.death_callback()
            #damaged all bees by self.damage
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.imposter_counter>=1:
            self.true_queen = False
        QueenAnt.imposter_counter+=1
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        new_spot = self.place
        if self.true_queen is True:
            while new_spot.exit is not None:
                
                if not type(new_spot.exit.ant)== QueenAnt:
                    if new_spot.exit.ant is not None and new_spot.exit.ant.queen_buffer is False:
                        new_spot.exit.ant.damage *=2
                        new_spot.exit.ant.queen_buffer = True
                        if new_spot.exit.ant.is_container and new_spot.exit.ant.contained_ant is not None and not new_spot.exit.ant.contained_ant.queen_buffer:
                            if not type(new_spot.exit.ant.contained_ant)== QueenAnt:
                                new_spot.exit.ant.contained_ant.damage *= 2
                                new_spot.exit.ant.contained_ant.queen_buffer = True
                new_spot = new_spot.exit
